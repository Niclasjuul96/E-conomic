...
import { GOOGLE_CONFIG, SHEET_CONFIG } from '../constants/google-config';
...
export class GoogleSheetsService {
   ...
   * Returns the range where data was appended
   */
  async appendTransactions(transactions: TransactionDetail[], spreadsheetId?: string): Promise<void> {
    ...
    try {
      ...
      console.log('[GoogleSheets] After deduplication:', newTransactions.length, 'new transactions to append');

      if (newTransactions.length === 0) {
        console.log('[GoogleSheets] ⓘ All transactions are duplicates. Nothing to append.');
        return;
      }
      ...
      const values = newTransactions.map((t) => [
        ...
        timestamp, // Add timestamp for duplicate detection
      ]);

      const range = `${SHEET_CONFIG.TRANSACTIONS_SHEET_NAME}!A${SHEET_CONFIG.HEADER_ROW_NUMBER + 1}`;
      console.log('[GoogleSheets] Appending to range:', range);
      console.log('[GoogleSheets] Values to append:', values.slice(0, 2)); // Log first 2 rows as sample

      const response = await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/${id}/values/${encodeURIComponent(range)}:append?valueInputOption=RAW`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to append transactions: ${error.error?.message}`);
      }

      const result = await response.json();
      console.log('[GoogleSheets] ✓ Appended', newTransactions.length, 'transactions (skipped', transactions.length - newTransactions.length, 'duplicates). Response:', result);
    } catch (error) {
      console.error('[GoogleSheets] ✗ Error appending transactions:', error);
      throw error;
    }
  }
